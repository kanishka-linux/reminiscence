...
urlpatterns = [
    ...
    url(r'^(?P<username>[\w\d.@+-]+)/subdir/(?P<directory>[\w\d\s.&@+-\/]+)/readpdf-(?P<pdf_pos>[\d]+\-[\d]+)$', navigate_pdfdir, name='navigate_subdirpdf'),
    ...
    url(r'^(?P<username>[\w\d.@+-]+)/(?P<directory>[\w\d\s.&@+-\/]+)/(?P<url_id>[\d]+)/readpdf-(?P<pdf_pos>[\d]+\-[\d]+)$', navigate_pdfdir, name='navigate_dirpdf'),
